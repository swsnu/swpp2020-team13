...
def goalList(request):
    if request.method == 'GET':
        ...
        if 'tags' in json.loads(body): # tags should be added after an intance is created
            ...
            new_goal.save()  
        response_dict = {'id': new_goal.id, 'title': new_goal.title, 'photo': new_goal.photo, 'user': new_goal.user.id, 'created_at': (new_goal.created_at).strftime('%Y-%m-%d %H:%M:%S'), 'updated_at' : (new_goal.updated_at).strftime('%Y-%m-%d %H:%M:%S'), 'deadline': (new_goal.deadline).strftime('%Y-%m-%d %H:%M:%S'), 'tags':[tag for tag in new_goal.tags.names()]}
